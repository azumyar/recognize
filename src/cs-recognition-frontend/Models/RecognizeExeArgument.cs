...
internal class RecognizeExeArgument {
	public static readonly int FormatVersion = 2025042700;
	...
	[Description("環境音ではないと判断する音圧の境界値。デフォルトでは0.0が設定されています。お使いのマイクによって感度は異なります。")]
	[DefaultValue(null)]
	[ArgAttribute("--mic_db_threshold")]
	public float? ArgMicDbThresholdV2 { get; set; }

	[Category(categoryMic)]
	[DisplayName("入力音無視秒数[秒]")]
	[Description("この秒数未満の入力は無視します。デフォルトでは0.0が設定されています。お使いのマイクによって感度は異なります。")]
	[DefaultValue(null)]
	[ArgAttribute("--mic_record_min_duration")]
	public float? ArgMicRecordMinDuration { get; set; }
