...
class AudioModel:
    def transcribe(self, _:np.ndarray[np.int16] | np.ndarray[np.real]) -> (str, any):
        ...
...
class AudioModelWhisper(AudioModel):
    def __init__(
        ...
        self.audio_model = wis.whisper.load_model(m, download_root=download_root).to(device)

    def transcribe(self, na:np.ndarray[np.int16] | np.ndarray[np.real]) -> (str, any):
