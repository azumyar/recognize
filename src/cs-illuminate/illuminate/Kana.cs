...
namespace Haru.Kei;
internal class KanaConv {
	private const string KuromojiDicFile = "kuromoji.kdic";
	private const string KanaDicFile = "english.dic";
	private UserDictionary? userDictionary;
	private Dictionary<string, string> kanaDic = new Dictionary<string, string>();

	public static KanaConv Current { get; } = new KanaConv();


	public KanaConv(string? assetDirectory = null) {
		var d = assetDirectory switch {
			string s => s,
			_ => Path.Combine(AppContext.BaseDirectory, "assets")
		};
		...
		try {
			foreach (var it in File.ReadAllLines((Path.Combine(d, KanaDicFile)))) {
				if (it.Any() && (it.Split('\t') is string[] s) && (s.Count() == 2)) {
					kanaDic.Add(s[0].ToLower(), s[1]);
				}
			}
		}
		catch (IOException) { }
		...
		try {
			userDictionary = new UserDictionary(new StringReader(File.ReadAllText(Path.Combine(d, KuromojiDicFile))));
		}
		catch (Exception) { }
	}
	...
	public string Convert(string text) {
		string kana(string text) {
			if (kanaDic.TryGetValue(text.ToLower(), out var result)) {
				return result;
			} else {
				return ToKana(text);
			}
		...
		using var tokenizer = new JapaneseTokenizer(reader, userDictionary, false, JapaneseTokenizerMode.SEARCH);
